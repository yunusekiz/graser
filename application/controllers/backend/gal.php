...
class gal extends CI_Controller {
	...
	public function addItemForm() 
	{
		if ($this->{$this->model_name}->readParentRow() != NULL)
			$this->parser_data['gal_cat_iteration'] = $this->{$this->model_name}->readParentRow();
		else
			$this->parser_data['gal_cat_iteration'] = array();
	...
	public function addItem()
	{
		...
		$gal_detail	= $this->input->post('detail');
		$cat_id	 		= $this->input->post('cat_id');

		if (($gal_title!='')&&($cat_id!='0')&&($gal_detail!='')) 
		{
			$item_css = strtolower(filterForeignChars($gal_title));
			...
			if ($image_up_and_resize == TRUE) 
			{
				...
				$insert_item_detail_to_db = $this->{$this->model_name}->insertNewItemDetail($cat_id,$gal_title,
																							$gal_detail,$item_css);
	...
	public function allItems()
	{
		...
		$this->parser->parse('backend_views/admin_header_view',$this->parser_data);
		$this->parser->parse('backend_views/admin_main_view',$this->parser_data);
		$this->parser->parse('backend_views/all_gal_by_cat_view',$this->parser_data);
		$this->parser->parse('backend_views/admin_footer_view',$this->parser_data);		
	}
	...
	public function allItemById($id,$name)
	{
		...
		$this->parser->parse('backend_views/admin_header_view',$this->parser_data);
		$this->parser->parse('backend_views/admin_main_view',$this->parser_data);
		$this->parser->parse('backend_views/all_gal_by_id_view',$this->parser_data);
		$this->parser->parse('backend_views/admin_footer_view',$this->parser_data);		
	}
	...
	public function updateItemDetailForm($id)
	{
		...
		$this->parser->parse('backend_views/admin_header_view',$this->parser_data);
		$this->parser->parse('backend_views/admin_main_view',$this->parser_data);
		$this->parser->parse('backend_views/update_gal_detail_view',$this->parser_data);
		$this->parser->parse('backend_views/admin_footer_view',$this->parser_data);			
	}
	...
	public function updateItemDetail()
	{
		...
		if ($cat_id != '0')
			...
			$gal_cat_id = $hidden_cat_id;

		$gal_title = $this->input->post('gal_title');
		$gal_detail = $this->input->post('gal_detail');

		if (($gal_title!='')&&($gal_detail!='')) 
		{
			$item_css = strtolower(filterForeignChars($gal_title));

			$update_item_detail = $this->{$this->model_name}->updateItemDetail($gal_cat_id,$item_id,$gal_title,
																			   $gal_detail,$item_css);
			if ($update_item_detail == TRUE) 
			{
				$message = 'Kayıt Güncelleme Başarılı..!';
				/*$return_path = 'allItemById/'.$gal_cat_id.'/'.$hidden_cat_name;*/
				$return_path = 'allItems';
