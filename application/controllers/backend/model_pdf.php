...
class model_pdf extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->model_name = 'model_pdf_model';	
		
		$this->item_big_photo_column_name = 'pdf_file_path';
		$this->parser_data['base'] = base_url();
		$this->parser_data['backend_base'] = base_url().'backend/';
		$this->change_item_photo_form_action = 'model_pdf/changeItemPhoto';
		...
		$this->img_lib_bootstrap_data = array(
												...
												'upload_path'		=>	'assets/images/model_pdf'
					 	 					 );
	}
	...
	public function addItemForm()
	{
		...
		$this->parser->parse('backend_views/add_model_pdf_view',$this->parser_data);
	...
	public function addItem()
	{
		...
		if (($title!='')) 
		{
			...
			$file_name = filterForeignChars($title);
			...
			if ($file_upload == TRUE) 
			{
				...
				$insert_item_detail_to_db = $this->{$this->model_name}->insertNewItemDetail($title);
	...
	public function allItems()
	{
		...
		$this->parser->parse('backend_views/admin_header_view',$this->parser_data);
		$this->parser->parse('backend_views/admin_main_view',$this->parser_data);
		$this->parser->parse('backend_views/all_model_pdf_view',$this->parser_data);
		$this->parser->parse('backend_views/admin_footer_view',$this->parser_data);		
	}
	...
	public function updateItemDetailForm($id)
	{
		...
		$this->parser->parse('backend_views/admin_header_view',$this->parser_data);
		$this->parser->parse('backend_views/admin_main_view',$this->parser_data);
		$this->parser->parse('backend_views/update_model_pdf_view',$this->parser_data);
		$this->parser->parse('backend_views/admin_footer_view',$this->parser_data);			
	}
	...
	public function updateItemDetail()
	{
		...
		if (($title!='')) 
		{
			$update_item_detail = $this->{$this->model_name}->updateItemDetail($item_id,$title);
	...
	public function changeItemPhoto()
	{
		...
		$this->img_name = filterForeignChars($records[0]['pdf_title']);
