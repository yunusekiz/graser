...
class gal_out extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->img_lib_bootstrap_data = array(
											  ...
											  'upload_path'		 =>	'assets/images/gal_out',
	...
	public function addItemForm()
	{
		...
		$this->parser->parse('backend_views/add_gal_out_view',$this->parser_data);
	...
	public function controlItem()
	{
		...
		if (($title!='')) 
		{
			$this->img_name = filterForeignChars($title);
			...
			if ($image_up_and_resize == TRUE) 
			{
				...
				$insert_item_detail_to_db = $this->gal_out_model->insertNewItemDetail($title);
				if ($insert_item_detail_to_db==TRUE) // item detail ler db ye insert edilmişse, item photo bilgilerini db ye insert eder 
				{
					$insert_item_photo_to_db = $this->gal_out_model->insertNewItemPhoto($big_img_data_for_db, 
	...
	{
		if ($this->gal_out_model->readRow()!=NULL) 
		{
			$this->parser_data['all_items'] = $this->gal_out_model->readRow();
		...
		$this->parser->parse('backend_views/admin_header_view',$this->parser_data);
		$this->parser->parse('backend_views/admin_main_view',$this->parser_data);
		$this->parser->parse('backend_views/all_gal_out_view',$this->parser_data);
		$this->parser->parse('backend_views/admin_footer_view',$this->parser_data);		
	}

	public function updateItemDetailForm($id)
	{
		$this->parser_data['item_detail'] = $this->gal_out_model->readRow($id);
		
		$this->parser->parse('backend_views/admin_header_view',$this->parser_data);
		$this->parser->parse('backend_views/admin_main_view',$this->parser_data);
		$this->parser->parse('backend_views/update_gal_out_detail_view',$this->parser_data);
		$this->parser->parse('backend_views/admin_footer_view',$this->parser_data);			
	}
	...
	public function updateItemDetail()
	{
		...
		if (($reference_title!='')) 
		{
			...
			$update_item_detail = $this->gal_out_model->updateItemDetail($reference_id, $reference_title);
	...
	public function changeItemPhotoForm($id, $photo_id)
	{
		...
		$this->parser_data['action'] = 'gal_out/changeItemPhoto';
	...
	public function deleteItem($id)
	{
		...
		{
			if ($this->gal_out_model->deleteRow($id) == TRUE)
			{
	...
	{
		$image_paths = $this->gal_out_model->readRow($id);

		$files = array($image_paths[0]['gal_out_big_photo'], $image_paths[0]['gal_out_thumb_photo']);
	...
	public function changeItemPhoto()
	{
		...
		$records 	= $this->gal_out_model->readRow($id);
		$this->img_name = filterForeignChars($records[0]['gal_out_title']);
		...
		if (($this->image_upload_resize_library->isSelectedAnyFile()==TRUE)&&($image_up_and_resize == TRUE)) 
		{
			...
			{
				if (($this->gal_out_model->deletePhotoRow($photo_id)==TRUE) &&
					($this->gal_out_model->insertNewItemPhoto($big_img_data_for_db,
