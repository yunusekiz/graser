...
class acty extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->model_name = 'acty_model';
		$this->item_big_photo_column_name = 'acty_big_photo';
		$this->item_thumb_photo_column_name = 'acty_thumb_photo';

		$this->change_item_photo_form_action = 'acty/changeItemPhoto';
		...
		$this->img_lib_bootstrap_data = array(
											  ...
											  'upload_path'		 =>	'assets/images/acty',
											  'image_name'		 =>	$this->img_name,
											  'big_img_width'	 =>	526,
											  'big_img_height'	 =>	395,
											  'thumb_img_width'	 =>	374,
											  'thumb_img_height' =>	206
					 	 					);
	}
	...
	public function addItemForm() 
	{
		...
		$this->parser->parse('backend_views/add_acty_view',$this->parser_data);
	...
	{
		$acty_date		 = $this->input->post('acty_date');
		$acty_title 	 = $this->input->post('acty_title');
		$acty_detail	 = $this->input->post('acty_detail');
		$acty_title_eng	 = $this->input->post('acty_title_eng');
		$acty_detail_eng = $this->input->post('acty_detail_eng');

		if (($acty_title!='')&&($acty_detail!='')&&($acty_date!='')) 
		{
			if ($acty_title_eng =='')
				$acty_title_eng = 'default_activity_title';
			if($acty_detail_eng =='')
				$acty_detail_eng = 'default_activity_detail';
			...
			if ($image_up_and_resize == TRUE) 
			{
				...
				$insert_item_detail_to_db = $this->{$this->model_name}->insertNewItemDetail($acty_date, $acty_title,
																							$acty_detail, $acty_title_eng,
																							$acty_detail_eng);
	...
	public function allItems()
	{
		...
		$this->parser->parse('backend_views/admin_header_view',$this->parser_data);
		$this->parser->parse('backend_views/admin_main_view',$this->parser_data);
		$this->parser->parse('backend_views/all_acty_view',$this->parser_data);
		$this->parser->parse('backend_views/admin_footer_view',$this->parser_data);		
	}
	...
	public function updateItemDetailForm($id)
	{
		...
		$this->parser_data['id'] = 'acty_id';
		
		$this->parser->parse('backend_views/admin_header_view',$this->parser_data);
		$this->parser->parse('backend_views/admin_main_view',$this->parser_data);
		$this->parser->parse('backend_views/update_acty_view',$this->parser_data);
		$this->parser->parse('backend_views/admin_footer_view',$this->parser_data);			
	}
	...
	public function updateItemDetail()
	{
		...
		if (($acty_title!='')&&($acty_detail!='')&&($acty_date!='')) 
		{
			...
			$update_item_detail = $this->{$this->model_name}->updateItemDetail($item_id,$acty_date,$acty_title,
																			   $acty_detail, $acty_title_eng,
																			   $acty_detail_eng);
